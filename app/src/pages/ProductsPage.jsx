...
import { BiCartAdd, BiEdit, BiSearch, BiPurchaseTag, BiTrash, BiCloudUpload } from 'react-icons/bi'
...
export default function ProductsPage() {
        ...
        reader.onload = async (event) => {
            try {
                ...
                try {
                    ...
                    throw new Error("Plik nie zawiera poprawnego formatu JSON.")
                }

                if (!Array.isArray(data)) {
                    throw new Error("Dane muszą być tablicą produktów (Array).")
                }
                ...
                alert("Błąd: " + (err.message || "Wystąpił problem"))
            } finally {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            alert("Błąd usuwania: " + (err.problem?.detail || err.message))
        }
    }
    ...
    const saveEdit = async () => {
        try {
            ...
            alert("Błąd aktualizacji: " + (err.problem?.detail || err.message))
        }
    }
    ...
    const handleOptimize = async (productId) => {
        ...
        try {
            ...
            if (!newDescription) {
                alert("Serwer zwrócił pusty opis. Sprawdź backend.");
                return;
            }
    ...
    return (
        <div className="container">
            <div className="d-md-flex justify-content-between align-items-center mb-5">
                ...
                {/* Przycisk dodawania widoczny tylko gdy baza NIE jest pusta */}
                {isEmployee && !isDatabaseEmpty && (
                    <button
                        className={`btn ${showCreateForm ? 'btn-secondary' : 'btn-success'} shadow-sm rounded-pill px-4`}
            ...
            {/* Filtry widoczne tylko gdy są produkty */}
            {!isDatabaseEmpty && (
                                        ...
                                        <div className="card-body">
                                            ...
                                            <div className="row g-3">
                                                <div className="col-md-4">
                                                    <label className="small text-muted">Nazwa</label>
                                                    <input className="form-control" value={editForm.name} onChange={e=>setEditForm({...editForm, name: e.target.value})} />
                                                </div>
                                                <div className="col-md-2">
                                                    <label className="small text-muted">Cena</label>
                                                    <input className="form-control" type="number" value={editForm.price} onChange={e=>setEditForm({...editForm, price: e.target.value})} />
                                                </div>
                                                <div className="col-md-2">
                                                    <label className="small text-muted">Waga</label>
                                                    <input className="form-control" type="number" value={editForm.weight} onChange={e=>setEditForm({...editForm, weight: e.target.value})} />
                                                </div>
                                                <div className="col-md-4">
                                                    <label className="small text-muted">Kategoria</label>
                                                    <select className="form-select" value={editForm.category} onChange={e=>setEditForm({...editForm, category: e.target.value})}>
                                                        {categories.map(c=><option key={c._id} value={c._id}>{c.name}</option>)}
                                                    </select>
                                                </div>
                                                <div className="col-12">
                                                    <label className="small text-muted">Opis</label>
                                                    <textarea className="form-control mb-2" rows="3" value={editForm.description} onChange={e=>setEditForm({...editForm, description: e.target.value})} />
                                                    <button className="btn btn-sm btn-outline-info" onClick={()=>handleOptimize(p._id)} disabled={isGenerating}>
                                                        {isGenerating ? 'Generowanie...' : '✨ Ulepsz opis z AI'}
                                                    </button>
                                                </div>
                                                <div className="col-12 d-flex gap-2 justify-content-end">
                                                    <button className="btn btn-secondary" onClick={()=>setEditId(null)}>Anuluj</button>
                                                    <button className="btn btn-success px-4" onClick={saveEdit}>Zapisz Zmiany</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="card h-100 shadow-hover border-0 overflow-hidden" style={{transition: 'transform 0.2s, box-shadow 0.2s'}}>
                                    ...
                                    <div className="card-body d-flex flex-column">
                                        <div className="d-flex justify-content-between align-items-start mb-2">
                                            <span className="badge bg-info text-dark bg-opacity-10 text-info border border-info border-opacity-25 rounded-pill">
                                                {p.category?.name || 'Inne'}
                                            </span>
                                            <span className="fw-bold text-success fs-5">{parseFloat(p.price).toFixed(2)} zł</span>
                                        </div>

                                        <h5 className="card-title fw-bold text-dark">{p.name}</h5>

                                        <div className="card-text text-muted small mb-4 flex-grow-1"
                                             dangerouslySetInnerHTML={{__html: p.description?.substring(0, 100) + (p.description?.length > 100 ? '...' : '')}}>
                                        </div>
